...
function generateSingleEliminationBracket(teamIds: string[]): {
    round: number; match_number: number; team_a_id: string | null; team_b_id: string | null; status: string;
}[] {
    ...
    const ordered: (string | null)[] = new Array(size);
...
function generateRoundRobinMatches(teamIds: string[]): {
    round: number; match_number: number; team_a_id: string; team_b_id: string; status: string;
}[] {
    const matches: any[] = [];
    let matchNum = 1;
    ...
    try {
        ...
        const registration = await prisma.tournamentRegistration.create({
            data: {
                ...
                players: {
                    create: player_ids.map((pid: string, i: number) => ({
                        profile_id: pid,
...
router.delete('/:id/register', requireAuth, async (req: any, res) => {
    try {
        const userId = req.user.id;
        ...
        if (!player) return res.status(404).json({ error: 'No estás registrado en este torneo' });
...
router.post('/', requireAuth, async (req: any, res) => {
    try {
        ...
        const {
            unit_id, name, sport, format, team_size, max_teams,
            registration_fee, prize_description, rules,
            start_date, end_date, registration_deadline, image_color,
        } = req.body;
...
router.post('/:id/generate-bracket', requireAuth, async (req: any, res) => {
    try {
        const perms = req.user.parsedPermissions;
        if (req.user.role !== 'titular' && !perms?.admin) {
            return res.status(403).json({ error: 'No autorizado' });
...
router.post('/:id/matches/:matchId/score', requireAuth, async (req: any, res) => {
    try {
        const perms = req.user.parsedPermissions;
        if (req.user.role !== 'titular' && !perms?.admin) {
            return res.status(403).json({ error: 'No autorizado' });
        ...
        if (!winner_id) return res.status(400).json({ error: 'Debes indicar el ganador' });
        ...
        if (tournament && tournament.format !== 'round_robin') {
            ...
            const pending = tournament.matches.filter(m =>
                m.id !== match.id && m.status !== 'completed'
            );
            if (pending.length === 0) {
                await prisma.tournament.update({
                    where: { id: tournament.id },
...
router.get('/:id/standings', async (req, res) => {
    try {
        ...
        const standings = tournament.registrations.map(reg => {
            ...
            const losses = tournament.matches.filter(m =>
                (m.team_a_id === reg.id || m.team_b_id === reg.id) && m.winner_id && m.winner_id !== reg.id
            ).length;

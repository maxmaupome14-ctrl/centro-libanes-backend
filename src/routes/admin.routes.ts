...
router.put('/staff/:id/services', async (req: any, res: any) => {
    ...
    try {
        ...
        await prisma.$transaction([
            prisma.staffService.deleteMany({ where: { staff_id: req.params.id } }),
            ...service_ids.map((sid: string) =>
                prisma.staffService.create({ data: { staff_id: req.params.id, service_id: sid } })
            ),
        ]);
...
router.get('/finance/summary', async (_req: any, res: any) => {
    try {
        ...
        const [
            ...
            prisma.maintenanceBilling.aggregate({
                where: { status: 'pendiente' },
                _sum: { amount: true },
                _count: true,
            }),
            // Active locker rentals
            prisma.lockerRental.aggregate({
                where: { status: 'activa' },
                _sum: { price: true },
                _count: true,
            }),
...
router.get('/reservations/today', async (_req: any, res: any) => {
    try {
...
router.get('/catalog/stats', async (_req: any, res: any) => {
    try {
        const [services, resources, activities, enrollments] = await Promise.all([
            ...
            prisma.resource.findMany({ include: { unit: { select: { short_name: true } } } }),
            ...
            prisma.activityEnrollment.count({ where: { status: 'activa' } }),
        ]);
...
router.get('/commissions', async (_req: any, res: any) => {
    try {
        ...
        // Group by staff
        const resByStaff: Record<string, any[]> = {};
        for (const r of reservations) {
            if (!r.staff_id) continue;
            if (!resByStaff[r.staff_id]) resByStaff[r.staff_id] = [];
...
router.get('/guests/today', async (_req: any, res: any) => {
    try {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
...
router.post('/qr/validate', async (req: any, res: any) => {
    const { code } = req.body;
    ...
    try {
        ...
        if (!membership) return res.status(404).json({ error: 'Membresía no encontrada', valid: false });

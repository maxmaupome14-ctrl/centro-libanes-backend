...
router.get('/featured/all', async (_req: any, res: any) => {
    try {
        const items = await prisma.featuredItem.findMany({ orderBy: { display_order: 'asc' } });
        return res.json(items);
    } catch (err: any) {
        return res.status(500).json({ error: err.message });
    }
...
router.get('/explore', async (_req: any, res: any) => {
    try {
...
router.get('/explore/all', async (_req: any, res: any) => {
    try {
        const items = await prisma.exploreItem.findMany({ orderBy: { display_order: 'asc' } });
        return res.json(items);
    } catch (err: any) {
        return res.status(500).json({ error: err.message });
    }
...
router.post('/explore', async (req: any, res: any) => {
    ...
    try {
        const item = await prisma.exploreItem.create({
...
router.get('/banners/all', async (_req: any, res: any) => {
    try {
        const banners = await prisma.banner.findMany({ orderBy: { display_order: 'asc' } });
        return res.json(banners);
    } catch (err: any) {
        return res.status(500).json({ error: err.message });
    }

...
function generatePassCode() {
    return crypto_1.default.randomBytes(4).toString('hex').substring(0, 6).toUpperCase();
}
...
router.post('/', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const monthCount = await prisma_1.default.guestPass.count({
            where: {
        ...
        let attempts = 0;
        while (attempts < 5) {
            const existing = await prisma_1.default.guestPass.findUnique({ where: { pass_code: passCode } });
            if (!existing)
                break;
        ...
        const pass = await prisma_1.default.guestPass.create({
            data: {
                invited_by_id: req.user.id,
                guest_name,
...
router.get('/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const pass = await prisma_1.default.guestPass.findUnique({
...
router.delete('/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const pass = await prisma_1.default.guestPass.findUnique({ where: { id: req.params.id } });
        if (!pass)
            return res.status(404).json({ error: 'Pase no encontrado' });
        if (pass.invited_by_id !== req.user.id) {
            return res.status(403).json({ error: 'No autorizado' });
        ...
        await prisma_1.default.guestPass.update({
            where: { id: req.params.id },
...
router.post('/validate', async (req, res) => {
    try {
        ...
        if (!pass)
            return res.status(404).json({ error: 'Código inválido', valid: false });
        const today = new Date();
        today.setHours(0, 0, 0, 0);

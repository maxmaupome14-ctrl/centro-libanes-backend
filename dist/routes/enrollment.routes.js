...
router.get('/my', auth_1.requireAuth, async (req, res) => {
    const user = req.user;
    try {
        const enrollments = await prisma_1.default.activityEnrollment.findMany({
...
router.post('/', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const existing = await prisma_1.default.activityEnrollment.findFirst({
        ...
        const enrollment = await prisma_1.default.activityEnrollment.create({
            data: {
                activity_id,
                profile_id: user.id,
...
router.delete('/:id', auth_1.requireAuth, async (req, res) => {
    ...
    const user = req.user;
    try {
        const enrollment = await prisma_1.default.activityEnrollment.findUnique({
            where: { id },
        ...
        await prisma_1.default.activityEnrollment.update({
            where: { id },

...
const getDefaultPermissions = (role, is_minor) => {
    if (role === 'titular') {
        return {
...
router.get('/:id/beneficiaries', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    try {
...
router.post('/:id/beneficiaries', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const newProfile = await prisma_1.default.memberProfile.create({
            data: {
                membership_id: id,
                role,
                first_name,
...
router.patch('/:id/beneficiaries/:pid', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const updatedProfile = await prisma_1.default.memberProfile.update({
            where: { id: pid },
            data: updateData
        });
        return res.json({
...
router.delete('/:id/beneficiaries/:pid', auth_1.requireAuth, async (req, res) => {
    const { pid } = req.params;
    const userPerms = req.user.parsedPermissions;
    if (!userPerms.can_manage_beneficiaries) {

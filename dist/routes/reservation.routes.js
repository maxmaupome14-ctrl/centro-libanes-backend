...
router.get('/slots', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (resource_id) {
            ...
            const slots = await Promise.all(allSlots.map(async (time) => {
                ...
                const bookedCount = await prisma_1.default.reservation.count({
                    where: {
                ...
                if (available <= 0)
                    status = 'full';
                else if (available <= Math.ceil(totalCourts * 0.3))
                    status = 'limited';
        ...
        if (service_id) {
            ...
            const staffSlots = await (0, reservation_service_1.getAvailableSlots)(service_id, dateStr);
            ...
            const slots = Array.from(allTimes).sort().map(time => {
                ...
                if (staffAvailable <= 0)
                    status = 'full';
                else if (staffAvailable === 1 && totalStaff > 1)
                    status = 'limited';
                return {
                    time,
                    status,

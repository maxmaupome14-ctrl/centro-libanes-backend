...
router.post('/create-intent', auth_1.requireAuth, async (req, res) => {
...
router.post('/:id/confirm', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        if (payment.status === 'completado')
            return res.json({ message: 'Pago ya confirmado', payment });
...
router.get('/:id/statement', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    try {
        const membership = await prisma_1.default.membership.findUnique({
            where: { id },

...
function setupCronJobs() {
    ...
    node_cron_1.default.schedule('5 0 * * *', async () => {
        ...
        const overdueBills = await prisma_1.default.maintenanceBilling.findMany({
            where: {
        ...
        for (const bill of overdueBills) {
            ...
            if (daysOverdue <= 10 && bill.status === 'pendiente') {
                ...
                for (const p of profiles) {
                    await prisma_1.default.reservation.updateMany({
                        where: {
                            profile_id: p.id,
    ...
    node_cron_1.default.schedule('0 0 15 * *', async () => {
        ...
        await (0, settlement_service_1.generateStaffSettlements)(periodStart, periodEnd);
    });
    // Last day of every month - generate staff settlements for 16th-end
    node_cron_1.default.schedule('0 0 28-31 * *', async () => {
        const now = new Date();
        const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
        if (now.getDate() !== lastDay)
            return; // Only run on actual last day
    ...
    node_cron_1.default.schedule('0 0 25 3,6,9,12 *', async () => {
        console.log('[Cron] Processing locker renewals...');
        const { currentQ, nextQ } = getQuarterStrings();
        await (0, locker_service_1.processLockerRenewals)(currentQ, nextQ);
    });
    ...
    node_cron_1.default.schedule('10 0 * * *', async () => {
        ...
        const grownUp = await prisma_1.default.memberProfile.findMany({
            where: {
                is_minor: true,
    ...
    node_cron_1.default.schedule('0 1 1 * *', async () => {
        ...
        const activeMemberships = await prisma_1.default.membership.findMany({
            where: { status: 'activa' }
        });
        for (const membership of activeMemberships) {
            // Check if bill already exists for this period
            const existing = await prisma_1.default.maintenanceBilling.findFirst({
            ...
            if (!existing) {
                ...
                await prisma_1.default.maintenanceBilling.create({
                    data: {
    ...
    node_cron_1.default.schedule('*/30 * * * *', async () => {
        ...
        const now = new Date();
        const fifteenAgo = (0, date_fns_1.addMinutes)(now, -15);
        const noShows = await prisma_1.default.reservation.findMany({
            where: {
                status: 'confirmada',
        ...
        if (noShows.length > 0)
            console.log(`[Cron] Marked ${noShows.length} no-shows`);
    });
    ...
    node_cron_1.default.schedule('0 8 * * *', async () => {
        ...
        const upcoming = await prisma_1.default.reservation.findMany({
    ...
    node_cron_1.default.schedule('0 7 * * *', async () => {
        ...
        tomorrow.setDate(tomorrow.getDate() + 1);
        const staffMembers = await prisma_1.default.staff.findMany({ where: { is_active: true } });
        for (const staff of staffMembers) {
            const appointments = await prisma_1.default.reservation.count({
    ...
    node_cron_1.default.schedule('*/30 * * * *', async () => {
        const now = new Date();
        const twoHoursFromNow = (0, date_fns_1.addMinutes)(now, 120);
        const windowEnd = (0, date_fns_1.addMinutes)(now, 150);
        const upcoming = await prisma_1.default.reservation.findMany({
            where: {
                status: 'confirmada',
    ...
    node_cron_1.default.schedule('0 2 1 * *', async () => {
        ...
        const memberships = await prisma_1.default.membership.findMany({ where: { status: 'activa' } });
        const now = new Date();
        const prevMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
        const period = `${prevMonth.getFullYear()}-${String(prevMonth.getMonth() + 1).padStart(2, '0')}`;
        for (const m of memberships) {
            const titular = await prisma_1.default.memberProfile.findFirst({
    ...
    node_cron_1.default.schedule('0 0 1 1,4,7,10 *', async () => {
    ...
    node_cron_1.default.schedule('0 0 3 1,4,7,10 *', async () => {
    ...
    node_cron_1.default.schedule('0 9 * * 1', async () => {
        ...
        const titulares = await prisma_1.default.memberProfile.findMany({
            where: { role: 'titular', is_active: true }
        });
        for (const titular of titulares) {
            const familyPayments = await prisma_1.default.payment.aggregate({
                _sum: { amount: true },
                _count: true,
